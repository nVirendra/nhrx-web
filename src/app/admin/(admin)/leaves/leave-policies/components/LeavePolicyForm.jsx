...
const mapApiLeaveTypesToState = (leaveTypes = []) => {
    const mapped = {};

    leaveTypes.forEach((lt) => {
        mapped[lt.leaveTypeCode] = {
            allocation: lt.allocation ?? 0,
            allocationCycle: lt.allocationCycleCode ?? "YEARLY",

            accrualBasis: lt.accrualBasisCode ?? "TIME",
            accrualMethod: lt.accrualMethodCode ?? "NONE",
            accrualValue: lt.accrualValue ?? 0,

            workingDaysThreshold: lt.workingDaysThreshold ?? 0,
            isContinuous: Boolean(lt.isContinuous),

            carryForward: Boolean(lt.carryForward),
            maxCarryForward: lt.maxCarryForward ?? 0,
            encashment: Boolean(lt.encashment),

            sandwichRule: Boolean(lt.sandwichRule),
            attachmentRequiredDays: lt.attachmentRequiredDays ?? 0,

            applicableFor: lt.applicableForCode ?? "ALL",
        };
    });
...
const LeavePolicyForm = ({
    ...
    useEffect(() => {
        ...
        setPolicyRules({
            allowLOP: initialData.allowLOP,
            applyLimitEnabled: initialData.applyLimitEnabled,
            applyBeforeDays: initialData.applyBeforeDays,
            applyAfterDays: initialData.applyAfterDays,
        });

        const mapped = mapApiLeaveTypesToState(initialData.leaveTypes);
        ...
        if (initialData.leaveTypes?.length) {
            setSelectedLeaveType(initialData.leaveTypes[0].leaveTypeCode);
        }
    }, [mode, initialData]);
    ...
    const buildPayload = () => ({
        ...
        applyAfterDays: policyRules.applyLimitEnabled
            ...
            : 0,
        leaveTypes: Object.entries(leavePolicies).map(([leaveType, config]) => ({
            leaveType,
            ...
            workingDaysThreshold: Number(config.workingDaysThreshold),
            isContinuous: Boolean(config.isContinuous),
            carryForward: Boolean(config.carryForward),
            maxCarryForward: Number(config.maxCarryForward),
            encashment: Boolean(config.encashment),
            sandwichRule: Boolean(config.sandwichRule),
            attachmentRequiredDays: Number(config.attachmentRequiredDays),
    ...
    return (
        <div className="p-6 space-y-8">
            ...
            <div className="flex justify-end">
                <Button size="lg" onClick={() => onSubmit(buildPayload())}>
